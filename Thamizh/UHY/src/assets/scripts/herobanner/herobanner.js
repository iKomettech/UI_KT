...
$(document).ready(function () {
    if ($('.homeBanner').length > 0) {
        ...
        function bannerimageht(){
            var imgHeight = $('.bannerpanel .homeBanner .bannerwrap .imgWrap').height()/4;
            $('.bannerpanel .homeBanner .bannerwrap .grid-container .content.right').css('top', imgHeight + 4);
            var contentPos = $('.bannerpanel .homeBanner .bannerwrap .grid-container .content.right').position();
            //console.log(contentPos);
            $('.bannertwocolumnpanel .flexwrap .leftWrap').css('margin-top', -imgHeight + 12);
            if ($(window).width() > 1800) {
                $('.bannertwocolumnpanel .flexwrap .rightWrap').css('margin-top', -contentPos.top - 164); 
             }
             else if ($(window).width() > 1700){
                $('.bannertwocolumnpanel .flexwrap .rightWrap').css('margin-top', -contentPos.top - 120);
             }
             else if ($(window).width() > 1600){
                $('.bannertwocolumnpanel .flexwrap .rightWrap').css('margin-top', -contentPos.top - 100);
             }else if ($(window).width() >= 1280){
                $('.bannertwocolumnpanel .flexwrap .rightWrap').css('margin-top', -contentPos.top - 30);
             }else if ($(window).width() < 1280){
                $('.bannertwocolumnpanel .flexwrap .leftWrap').css('margin-top', -imgHeight - 88);
                $('.bannertwocolumnpanel .flexwrap .rightWrap').css('margin-top', -contentPos.top - 90);
             }
        }
...
$(document).ready(function () {  
    $(".bannerpanel .bannerwrap .filter-img.onloadVideo .video-thumb-image").each(function () {
        $(this).trigger('click');
        setTimeout(function() { 
            $(".play-bt").trigger("click");
        }, 2000);
        $('.bannervideosection .video-controller .play-bt').css("display", "none");
        setTimeout(function () {
            $('.bannervideosection .video-controller .play-bt').trigger('click');
        }, 800);
    });
    ...
    $('.mute-bt').on('click', function () {
        var dataIndex = $(".bannerpanel").find('.youtubevideopanel').data('index');
        if (dataIndex >= 0 ) {
    ...
    $(".play-bt-vimeo").click(function () {
        ...
        $(".pause-bt-vimeo").show();
        $(".bannerpanel").find('iframe').show();
        $(this).closest('.bannerpanel').find('.vimeo-video-src').trigger('click');        
        var dataIndex = $(".bannerpanel").find('.vimeovidoepanel').data('index'); 
        if (dataIndex >= 0 ) {
